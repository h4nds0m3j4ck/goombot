...
async def scrape_pc_gamer(limit=3):
    all_news = []
    try:
...
async def scrape_playstation_blog(limit=3):
    all_news = []
    try:
...
async def scrape_the_verge(limit=3):
    all_news = []
    try:
        # Try using Playwright
...
async def scrape_gamer_rant_with_playwright(limit=3):
    all_news = []
    try:
        async with async_playwright() as p:
...
async def scrape_top_gaming_news(limit=3):
